...
class CorePermissionHandler {
  static Future<void> requestPermissions() async {
    try {
      ...
      // Request to ignore battery optimization (CRITICAL for connection stability)
      ...
      if (Platform.isAndroid) {
        ...
        if (status.isDenied) {
          LoggerDebug.logger.w(
            'Requesting battery optimization bypass for connection stability',
          );
          await Permission.ignoreBatteryOptimizations.request();
        }
        LoggerDebug.logger.i(
          'Battery optimization status: ${await Permission.ignoreBatteryOptimizations.status}',
        );
      }
    } catch (e) {
      LoggerDebug.logger.e('Error requesting battery optimization bypass: $e');
    }
  }
}
